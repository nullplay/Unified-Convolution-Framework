...
struct Iterator::Content {
  ...
  struct Window {
    ...
    ir::Expr windowVar;
    ir::Expr lo;
    ir::Expr off; //For Affine
    ir::Expr hi;
    ir::Expr stride;
    Window(ir::Expr _lo, ir::Expr _off, ir::Expr _hi, ir::Expr _stride, ir::Expr _windowVar) :
      windowVar(_windowVar), lo(_lo), off(_off), hi(_hi), stride(_stride) {};
  };
...
Stmt Iterator::getAppendEdges(const Expr& pPrev, const Expr& pBegin, 
                              const Expr& pEnd, const bool isCompound) const {
  taco_iassert(defined() && content->mode.defined());
...
bool Iterator::isAffineWindowed() const {
  if (this->content->window != nullptr) {
    ir::Expr offset = this->content->window->off;
    bool isOffsetZero = isa<ir::Literal>(offset);
    return !isOffsetZero;
  } else {
    return false;
  }
...
ir::Expr Iterator::getWindowOffset() const {
  taco_iassert(this->isWindowed());
  return this->content->window->off;
}
...
void Iterator::setWindowBounds(ir::Expr lo, ir::Expr off, ir::Expr hi, ir::Expr stride) {
  auto windowVarName = this->getIndexVar().getName() + this->getMode().getName() + "_window";
  auto wvar = ir::Var::make(windowVarName, Int());
  this->content->window = std::make_unique<Content::Window>(Content::Window(lo, off, hi, stride, wvar));
}
...
: Iterators()
{
  //std::cout << "Iterators start " << std::endl;
  ...
  match(stmt,
    function<void(const AccessNode*)>([&](auto n) {
      ...
      //std::cout << n->tensorVar << " " << tensorIR << std::endl;
  ...
  for (auto it : content->modeIterators) {
    //cout << it.first << endl; 
  }
  ...
  for (ModeFormatPack modeTypePack : format.getModeFormatPacks()) {
    ...
    for (auto& modeType : modeTypePack.getModeFormats()) {
      ...
      if (access.isModeWindowed(modeNumber)) {
        if (access.isModeAffineWindowed(modeNumber)) { 
          std::vector<IndexVar> loVars = access.getAffineWindowVars(modeNumber);
          std::vector<int> loStrides = access.getAffineWindowStrides(modeNumber);
          auto lo = ir::Literal::make(access.getWindowLowerBound(modeNumber));
          auto off = ir::Literal::make(access.getWindowLowerBound(modeNumber));
          for (size_t i = 0; i < loVars.size(); i++) {
            IndexVar& affineVar = loVars[i];
            int affineStride = loStrides[i];
            off = ir::Add::make(off, 
                               ir::Mul::make(ir::Literal::make(affineStride),
                                             ir::Var::make(affineVar.getName(),
                                                           taco::Int32,
                                                           false, false)
                                            ));
            //off = ir::Add::make(off, 
            //                   ir::Mul::make(ir::Literal::make(affineStride),
            //                                 content->modeIterators[affineVar].getCoordVar()
            //                                 //ir::Var::make(affineVar.getName(),
            //                                 //              taco::Int32,
            //                                 //              false, false)
            //                                 ));
          ...
          iterator.setWindowBounds(lo, off, hi, stride);
        } else {
          auto lo = ir::Literal::make(access.getWindowLowerBound(modeNumber));
          auto hi = ir::Literal::make(access.getWindowUpperBound(modeNumber));
          auto stride = ir::Literal::make(access.getStride(modeNumber));
          iterator.setWindowBounds(lo, lo, hi, stride);
        }
      }
