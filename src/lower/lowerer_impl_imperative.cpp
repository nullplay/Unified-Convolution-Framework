...
static void createReducedValueVars(const vector<Access>& inputAccesses,
  ...
  for (const auto& access : inputAccesses) {
    ...
    //reducedValueVars->insert({access, Literal::make(1)});
  }
}
                   ...
                   bool assemble, bool compute, bool pack, bool unpack)
{
  ...
  //cout << "---[Iterators]---" << endl;
  for (auto it : iterators.modeIterators()) {
...
Stmt LowererImplImperative::lowerForall(Forall forall)
{
  ...
  //cout << "[lowerForall " << forall.getIndexVar() << "] "<< caseLattice << " - " << forall.getIndexVar()<< endl;
  ...
  if (caseLattice.iterators().size() == 1 && caseLattice.iterators()[0].isUnique()) {
    ...
    //cout << "[Forall " << forall.getIndexVar() << "]" << endl;
    ...
    //cout << "[lowerForall " << forall.getIndexVar() << "] "<< "Forall Dimension Iteration " << endl;
    //cout << "-- " << point.locators().size() << " " ;
    //for (auto it : point.locators()) { cout << it << " " ;}
    //cout << endl;
    ...
    // Emit position iteration loop
      ...
      //cout << "[lowerForall " << forall.getIndexVar() << "] "<< "Forall Position Iteration" << endl;
...
//      taco_not_supported_yet
  ...
  // Emit general loops to merge multiple iterators
    ...
    //cout << "[lowerForall " << forall.getIndexVar() << "] Entering merge lattice lowering stage of " << endl;
    ...
    //cout << "[ForallMerge " << forall.getIndexVar() << "]" << endl;
    //cout << "[lowerForall " << forall.getIndexVar() << "] "<< "Forall CoIteration" << endl;
                                    ...
                                    MergeStrategy mergestrategy)
{
  ...
  MergeLattice loopLattice = caseLattice.getLoopLattice();
  //cout << "[lowerMergeLattice " <<coordinateVar<< "] BEGINS " <<  loopLattice << endl;
  ...
  for (MergePoint point : loopLattice.points()) {
    ...
    //cout << "[lowerMergeLattice " <<coordinateVar<< "] Iterating the subLattice " << sublattice << " of " << loopLattice << endl;
    ...
    //cout << "[lowerMergeLattice " <<coordinateVar<< "] subLattice's Stmt " << mergeLoop << endl;
  }
  Stmt mergeLoops = Block::make(mergeLoopsVec);

  //cout << "[lowerMergeLattice] mergeLoops Stmt : \n " << mergeLoops << endl;

  // Append position to the pos array
  Stmt appendPositions = generateAppendPositions(appenders);
  //cout << "[lowerMergeLattice " <<coordinateVar<< "] ENDS " << endl;
                                  ...
                                  MergeStrategy mergeStrategy)
{
  ...
  //cout << "[lowerMergePoint] Entering lowerMergePoint" << endl;
  ...
  //cout << "[lowerMergePoint] loadPosIterCoordinates \n" << loadPosIterCoordinates << endl;
  ...
  //cout << "[lowerMergePoint] indexSetStmts \n" << ir::Block::make(indexSetStmts) << endl;
  ...
  //cout << "[lowerMergePoint] resolvedCoordinate \n" << resolvedCoordinate << endl;

  // Locate positions
  Stmt loadLocatorPosVars = declLocatePosVars(locators);
  //cout << "[lowerMergePoint] loadLocatorPosVars \n" << loadLocatorPosVars << endl;
  ...
  Stmt deduplicationLoops = reduceDuplicateCoordinates(coordinate, dupIters,
                                                       alwaysReduce);
  //cout << "[lowerMergePoint] deduplicationLoops \n" << deduplicationLoops << endl;
  ...
  //cout << "[lowerMergePoint] caseStmts \n" << caseStmts << endl;
  ...
  //cout << "[lowerMergePoint] incIteratorVarStmts \n" << incIteratorVarStmts << endl;
                                  ...
                                  MergeStrategy mergeStrategy)
{
  ...
  if (loopLattice.iterators().size() == 1) {
    ...
    for (MergePoint point : loopLattice.points()) {
      ...
      //cout << "[lowerMergeCases] " << point << " of " << loopLattice << " - " << zeroedStmt << endl;
...
Stmt LowererImplImperative::lowerForallBody(Expr coordinate, IndexStmt stmt,
  ...
  //cout << "[lowerForallBody] " << coordinate << " " << declLocatorPosVars << endl;
...
Stmt LowererImplImperative::lowerWhere(Where where) {
  ...
  return Block::make(initializeTemporary, producer, ir::Stmt(), consumer,  freeTemporary);
}
...
Expr LowererImplImperative::lowerAccess(Access access) {
  ...
  if (isScalar(var.getType())) {
    //std::cout << "isScalar " << var << " " << getTensorVar(var) << std::endl;
    return getTensorVar(var);
  }

  if (!getIterators(access).back().isUnique()) {
    //std::cout << "isUnique " << var << " " << getReducedValueVar(access) << std::endl;
...
Stmt LowererImplImperative::defineScalarVariable(TensorVar var, bool zero) {
  ...
  init = var.getType().getIsScalar() ? ir::Literal::make(1) : init;
  tensorVars.find(var)->second = varValueIR;
  return VarDecl::make(varValueIR, init, var.getType().getIsScalar());
}
...
Stmt LowererImplImperative::declLocatePosVars(vector<Iterator> locators) {
  ...
  for (Iterator& locator : locators) {
    ...
    if (doLocate) {
      ...
      do {
        //get all coordinate expression above this locateIterator.
        ...
        //std::cout << "[DeclarePosVar] " << locateIterator.getPosVar() << " " << locate.getResults()[0] << " / ";
        //for (auto c : coords) { std::cout << c << " " ; }
        //std::cout << std::endl;
        
        // If the WindowedIndexVar(Affine) tries to exceed the dimension of a mode,
        // a guard should avoid such a behavior.
        if (locateIterator.isAffineWindowed()) {
          // A[strd * k + off] : Affine 

          // modeDimension = A's shape
          // 0 <= strd*k + off < modeDimension  --(1)
          int modeDimension = locateIterator.getMode().getSize().getSize();

          // k's dimension
          // 0 <= k < |K| -- (zero lower-bound is due to bound inference)
          // off <= strd*k+off < strd*|K|+off   --(2) (Assume strd > 0)

          // interval analysis with (1) and (2)
          // max(0, off) <= strd * k + off < min(modeDimension, strd * |K| + off)
         
          // lbAffineOffset <= off < ubAffineOffset
          Expr affineOffset = locateIterator.getWindowOffset(); 
          auto boundsAffineOffset = boundsOfAffineExpr(affineOffset);
          int lbAffineOffset = boundsAffineOffset.first;
          int ubAffineOffset = boundsAffineOffset.second;

          // if max(0,off) = 0 (off < 0), it means iteration without a guard will cause segfault.
          // optimization 1) If lowerbound of off is larger than zero, we don't need guard.
          if (lbAffineOffset >= 0) {
          
          } else {
             Stmt guard = ir::IfThenElse::make(
                ir::Lt::make(coords[coords.size()-1], ir::Literal::make(0)),
                ir::Continue::make(true)
                );
             result.push_back(guard);
          }

          // if min(modeDimension, strd*|K|+off) = modeDimension, 
          // iteration without a guard will cause segfault.
          // strd * |K| + off > modeDimension : Need a guard
          // optimization 2) If upperbound of off is smaller than (modeDimension - strd*|K|),
          // we don't need a guard.
          auto baseStrideLiteral = to<ir::Literal>(locateIterator.getStride()); 
          auto baseStride = baseStrideLiteral->getValue<int>();
          IndexVar baseVar = locateIterator.getIndexVar(); 
          int ubBase;
          for (auto& it : dimensions) {
            if (it.first.getName() == baseVar.getName()) {
              auto simplifiedUpperBound = ir::simplify(it.second);
              taco_iassert(isa<ir::Literal>(simplifiedUpperBound));
              auto literalUpperBound = to<ir::Literal>(simplifiedUpperBound);
              ubBase = literalUpperBound->getValue<int>();
            }
          }
          if (ubAffineOffset <= modeDimension - baseStride * ubBase) {
          } else {
             Stmt guard = ir::IfThenElse::make(
                ir::Gte::make(coords[coords.size()-1], 
                              ir::Literal::make(modeDimension)),
                ir::Break::make()
                );
            result.push_back(guard);
          }
...
Stmt LowererImplImperative::reduceDuplicateCoordinates(Expr coordinate,
  ...
  for (Iterator& iterator : iterators) {
    ...
    if (iterator.isWindowed()) {
      Expr canonicalSpace = projectWindowedPositionToCanonicalSpace(iterator, posAccess.getResults()[0]);
      isDuplicate = Eq::make(canonicalSpace, coordinate);
    }
...
Stmt LowererImplImperative::codeToInitializeIteratorVar(Iterator iterator, vector<Iterator> iterators, vector<Iterator> rangers, vector<Iterator> mergers, Expr coordinate, IndexVar coordinateVar) {
  ...
  if (iterator.hasPosIter()) {
    ...
    if (iterator.getParent().isRoot() || iterator.getParent().isUnique()) {
      ...
      if (any(rangers,
        ...
        if (binarySearchTarget != underivedBounds[coordinateVar][0]) {
          ...
          result.push_back(
                    ...
                    VarDecl::make(iterVar, ir::Call::make("taco_linearSearchAfter", binarySearchArgs, iterVar.type())));
        }
        else {
      ...
      auto endBound = bounds[1];
      // End position is handled during two-finger while loop.
      //if (iterator.isWindowed()) {
      //    endBound = this->searchForEndOfWindowPosition(iterator, bounds[0], bounds[1]);
      //}
      result.push_back(VarDecl::make(endVar, endBound));
    } else {
      taco_iassert(iterator.isOrdered() && iterator.getParent().isOrdered());
      taco_iassert(iterator.isCompact() && iterator.getParent().isCompact());
      ...
      if (!iterator.isWindowed()) {
        ...
        auto bound = this->searchForStartOfWindowPosition(iterator, startBounds[0], endBounds[1]);
        result.push_back(VarDecl::make(iterVar, bound));
      }
...
Stmt LowererImplImperative::generateAppendPositions(vector<Iterator> appenders) {
  ...
  if (generateAssembleCode()) {
    for (Iterator appender : appenders) {
      ...
      result.push_back(appender.getAppendEdges(parentPos, beginPos, pos, true));
    }
  }
  //std::cout << "[Jaeyeon Debug2] " << (result.empty() ? Stmt() : Block::make(result)) << std::endl;
  return result.empty() ? Stmt() : Block::make(result);
}
...
Expr LowererImplImperative::searchForStartOfWindowPosition(Iterator iterator, ir::Expr start, ir::Expr end) {
    ...
    //if (iterator.isAffineWindowed()) {
      // A[strd * k + off] : Affine 

      // modeDimension = A's shape
      // 0 <= strd*k + off < modeDimension  --(1)
      int modeDimension = iterator.getMode().getSize().getSize();
      ...
      // lbAffineOffset <= off < ubAffineOffset
      Expr affineOffset = iterator.getWindowOffset(); 
      auto boundsAffineOffset = boundsOfAffineExpr(affineOffset);
      int ubAffineOffset = boundsAffineOffset.second;

      // if max(0,off) = 0 (off < 0), it means we don't need to do a search and just emit start of pos array.
      // optimization 1) If upperbound of off is smaller than zero, we don't need search.
      if (ubAffineOffset <= 0) {
        return start;  
      }  
    //} 
    ...
    vector<Expr> args = {
            ...
            iterator.getWindowOffset(),
    };
    
    //return ir::Call::make("taco_binarySearchAfter", args, Datatype::UInt64);
    return ir::Call::make("taco_linearSearchAfter", args, Datatype::UInt64);
}


Expr LowererImplImperative::searchForEndOfWindowPosition(Iterator iterator, ir::Expr start, ir::Expr end) {
    taco_iassert(iterator.isWindowed());
    //ir::Expr target = ir::Add(ir::Sub(iterator.getWindowOffset(), iterator.getWindowLowerBound()), iterator.getWindowUpperBound());
    //if (iterator.isAffineWindowed()) {
      // A[strd * k + off] : Affine 
      ...
      auto boundsAffineOffset = boundsOfAffineExpr(affineOffset);
      int lbAffineOffset = boundsAffineOffset.first;
      auto baseStrideLiteral = to<ir::Literal>(iterator.getStride()); 
      auto baseStride = baseStrideLiteral->getValue<int>(); //strd
      IndexVar baseVar = iterator.getIndexVar(); 
      int ubBase; //|K|
      for (auto& it : dimensions) {
        if (it.first.getName() == baseVar.getName()) {
          auto simplifiedUpperBound = ir::simplify(it.second);
      ...
      // if min(modeDimension, strd*|K|+off) = modeDimension (strd*|K|+off > modeDimension), 
      // it means we don't need to do a search and just emit end of pos array.
      // optimization 2) If lowerbound of off is larger than modeDimension - strd*|K|, we don't need search.
      if (lbAffineOffset >= modeDimension-baseStride*ubBase) {
        return end;  
      }
      else {
        vector<Expr> args = {
                ...
                ir::Add::make(ir::Literal::make(ubBase*baseStride), affineOffset),
        };
        //return ir::Call::make("taco_binarySearchAfter", args, Datatype::UInt64);
        return ir::Call::make("taco_linearSearchAfter", args, Datatype::UInt64);
      }
...
Stmt LowererImplImperative::upperBoundGuardForWindowPosition(Iterator iterator, ir::Expr access) {
  ...
  //access is canonical space index
...
Stmt LowererImplImperative::strideBoundsGuard(Iterator iterator, ir::Expr access, bool incrementPosVar) {
  ...
  return ir::IfThenElse::make(
      ir::Neq::make(ir::Rem::make(ir::Sub::make(access, iterator.getWindowOffset()), iterator.getStride()), ir::Literal::make(0)),
      cont
  );
}
...
Expr LowererImplImperative::projectWindowedPositionToCanonicalSpace(Iterator iterator, ir::Expr expr) {
  return ir::Div::make(ir::Sub::make(expr, iterator.getWindowOffset()), iterator.getStride());
}


Expr LowererImplImperative::projectCanonicalSpaceToWindowedPosition(Iterator iterator, ir::Expr expr) {
  return ir::Add::make(ir::Mul::make(expr, iterator.getStride()), iterator.getWindowOffset());
}


pair<int, int> LowererImplImperative::boundsOfAffineExpr(ir::Expr expr) {
  struct genBounds : public IRRewriter {
    using IRRewriter::visit;
    const std::map<IndexVar, ir::Expr>& dimensions;
    bool lowerBound;

    genBounds(const std::map<IndexVar, Expr>& dimensions, bool lowerBound)
        : dimensions(dimensions), lowerBound(lowerBound) {}

    //void visit(const Var* op) {
    //  for (auto& it : dimensions) {
    //    if (it.first.getName() == op->name) {
    //      expr = ir::Sub::make(it.second, ir::Literal::make(1));
    //      break;
    //    }
    //  }
    //}
    ...
    void visit(const ir::Mul* op) {
      Expr a = op->a;
      Expr b = op->b;
      taco_iassert(isa<ir::Literal>(a));
      taco_iassert(isa<ir::Var>(b));
      
      auto lita = to<ir::Literal>(a);
      int litaval = lita->getValue<int>(); //Stride
      auto varb = to<ir::Var>(b);
      for (auto& it : dimensions) {
        if (it.first.getName() == varb->name) {
          auto lbDimension = ir::Literal::make(0);
          auto ubDimension = ir::Sub::make(it.second, ir::Literal::make(1));
          if (lowerBound) {
            if (litaval > 0) {
              expr = ir::Mul::make(a, lbDimension);
            } else {
              expr = ir::Mul::make(a, ubDimension);
            }
          } else {
            if (litaval > 0) {
              expr = ir::Mul::make(a, ubDimension);
  ...
  int lbExpr = to<ir::Literal>(ir::simplify(genBounds(dimensions,true).rewrite(expr)))->getValue<int>();
  int ubExpr = to<ir::Literal>(ir::simplify(genBounds(dimensions,false).rewrite(expr)))->getValue<int>();
  return {lbExpr, ubExpr};
}
