...
void TensorBase::setScalar() {
  content->tensorVar.setScalar(true);
}
  ...
  AccessTensorNode(TensorBase tensor, const std::vector<std::shared_ptr<IndexVarInterface>>& indices)
    ...
    for (size_t i = 0; i < indices.size(); i++) {
      ...
      IndexVarInterface::match(var, [&](std::shared_ptr<IndexVar> ivar) {
        ...
        this->windowedModes[i].isAffine = wvar->isAffineWindow();
        if (wvar->isAffineWindow()) {
          this->windowedModes[i].loVars = wvar->getAffineLoVars();
          this->windowedModes[i].loStrides = wvar->getAffineLoStrides();
        }
...
void* TensorBase::getFuncPtr(std::string name) {
  return this->content->module->getFuncPtr(name); 
}
