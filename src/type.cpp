...
// class TensorType
Type::Type() : dtype(type<double>()), isScalar(false) {
}

Type::Type(Datatype dtype, Shape shape) : dtype(dtype), shape(shape), isScalar(false) {
}
...
void Type::setIsScalar(bool isScalar_) {
  isScalar = isScalar_;
}
...
bool operator==(const Type& a, const Type& b) {
  ...
  if (a.getIsScalar() != b.getIsScalar()) {
    return false;
  }
...
bool isScalar(const Type& type) {
  return (type.getOrder() == 0) || type.getIsScalar();
}
