...
bool Access::isModeAffineWindowed(int mode) const {
  taco_iassert(this->isModeWindowed(mode));
  return getNode(*this)->windowedModes.at(mode).isAffine;
}
...
std::vector<IndexVar> Access::getAffineWindowVars(int mode) const {
  taco_iassert(this->isModeWindowed(mode));
  return getNode(*this)->windowedModes.at(mode).loVars;
}

std::vector<int> Access::getAffineWindowStrides(int mode) const {
  taco_iassert(this->isModeWindowed(mode));
  return getNode(*this)->windowedModes.at(mode).loStrides;
}
...
Assignment BoundInference(Assignment assignment) {
  //const AssignmentNode* node = getNode(assignment);
  map<IndexVar, pair<int,int>> indexVarDomains;

  match(assignment,
    std::function<void(const AssignmentNode*)>([&indexVarDomains](const AssignmentNode* op) {
      Access access = op->lhs;
      auto& type = access.getTensorVar().getType();
      for (size_t i = 0; i < access.getIndexVars().size(); i++) {
        IndexVar var = access.getIndexVars()[i];
        int dimension = type.getShape().getDimension(i).getSize();
        if (access.isModeWindowed(i) && !access.isModeAffineWindowed(i)) {
          int lo = access.getWindowLowerBound(i);
          int stride = access.getStride(i);
          int indexUpperBound = (dimension-lo)/stride; 
          int indexLowerBound = -(lo/stride); 

          // LowerBound is handled in runtime. 
          if (!util::contains(indexVarDomains, var)) {
            indexVarDomains.insert({var, {indexLowerBound, indexUpperBound}});
          }
          else {
            int intersectLB = std::max(indexLowerBound, indexVarDomains[var].first);
            int intersectUB = std::min(indexUpperBound, indexVarDomains[var].second);
            indexVarDomains[var] = {intersectLB, intersectUB};
          }
        }
        if (!access.isModeWindowed(i) && !access.isModeIndexSet(i)) {
          if (!util::contains(indexVarDomains, var)) {
            indexVarDomains.insert({var, {0, dimension}});
          }
          else {
            int intersectLB = std::max(0, indexVarDomains[var].first);
            int intersectUB = std::min(dimension, indexVarDomains[var].second);
            indexVarDomains[var] = {intersectLB, intersectUB};
          }
        }
      }
    ...
    function<void(const AccessNode*)>([&indexVarDomains](const AccessNode* op) {
      ...
      for (size_t i = 0; i < access.getIndexVars().size(); i++) {
        IndexVar var = access.getIndexVars()[i];
        //cout << op->tensorVar.getName() << " " << access.isModeWindowed(i) << " " << access.isModeIndexSet(i) << endl;
        int dimension = type.getShape().getDimension(i).getSize();
        if (access.isModeWindowed(i) && !access.isModeAffineWindowed(i)) {
          int lo = access.getWindowLowerBound(i);
          int stride = access.getStride(i);
  ...
  struct WindowedVarRewriter : public IndexNotationRewriter {
    using IndexNotationRewriter::visit;
    map<IndexVar, pair<int,int>> varDomains;

    WindowedVarRewriter(map<IndexVar,pair<int,int>> varDomains)
    ...
    void visit(const AccessNode* op) {
      AccessNode* newNode = new AccessNode(op->tensorVar, op->indexVars, {}, false);
      //Deep Copy
      newNode->tensorVar = op->tensorVar;
      newNode->indexVars = op->indexVars;
      newNode->windowedModes = op->windowedModes;
      newNode->indexSetModes = op->indexSetModes;
      newNode->isAccessingStructure = op->isAccessingStructure;
      for (size_t i = 0; i < op->indexVars.size(); i++) {
        auto bound = varDomains[op->indexVars[i]];
        if (op->windowedModes.find(i) != op->windowedModes.end()) { // WindowedIndexVar
          AccessWindow window = op->windowedModes.at(i);
          if (!window.isAffine) {
            int newlo = bound.first * window.stride + window.lo;
            int newhi = bound.second * window.stride + window.lo;
            newNode->windowedModes.at(i).lo = newlo;
            newNode->windowedModes.at(i).hi = newhi;
          }
          else if (window.isAffine) {
            int lowest = 0;
            vector<IndexVar> loVars = window.loVars;
            vector<int> loStrides = window.loStrides;
            for (size_t idx = 0; idx < loStrides.size(); idx++) {
              lowest += loStrides[idx] * varDomains[loVars[idx]].first;
            }
            int newlo = bound.first * window.stride + window.lo + lowest;
            int newhi = bound.second * window.stride + window.lo + lowest;
            newNode->windowedModes.at(i).lo = newlo;
            newNode->windowedModes.at(i).hi = newhi;
          }
        }
          ...
          aw.loStrides = {};
          aw.isAffine = false;
          newNode->windowedModes.insert({i, aw}); 
        }
      }
      expr = newNode;
      //IndexNotationRewriter::visit(node);
  ...
  auto rewriter = WindowedVarRewriter(indexVarDomains);
  IndexStmt stmt = rewriter.rewrite(assignment);

  return to<Assignment>(stmt);
}

static void check(Assignment assignment) {
  //cout << "[Check] " << assignment << endl;
  //cout << "[Check2] " << BoundInference(assignment) << endl;
  //assignment = BoundInference(assignment);
  ...
  match(*this,
    std::function<void(const AssignmentNode*)>([&indexVarDomains](const AssignmentNode* op) {
      ...
      for (size_t i = 0; i < access.getIndexVars().size(); i++) {
        ...
        if (access.isModeWindowed(i) && !access.isModeAffineWindowed(i)) {
          if (!util::contains(indexVarDomains, var)) {
            indexVarDomains.insert({var, Dimension(access.getWindowSize(i))});
          }
        }
        if (!access.isModeWindowed(i) && !access.isModeIndexSet(i)) {
          if (!util::contains(indexVarDomains, var)) {
            indexVarDomains.insert({var, type.getShape().getDimension(i)});
          }
        }
      }
    }),
...
IndexStmt IndexStmt::reorder(taco::IndexVar i, taco::IndexVar j) const {
  ...
  transformed = makeNotationConcordant(transformed);
  return transformed;
}
...
const std::vector<IndexVar>& Assignment::getFreeVars() const {
  ...
  //auto lhs = getLhs();
  //auto& indexVars = getNode(lhs)->indexVars;
  //std::set<IndexVar> result(indexVars.begin(), indexVars.end());
  //for (auto& it : getNode(lhs)->windowedModes) {
  //  auto& affineVars = it.second.loVars;
  //  result.insert(affineVars.begin(), affineVars.end());  
  //}
  //static std::vector<IndexVar> resultVector(result.begin(), result.end());
...
std::vector<IndexVar> Assignment::getReductionVars() const {
  ...
  match(getRhs(),
    ...
    //for (auto& it : op->windowedModes) {
    //  auto& affineVars = it.second.loVars;
    //  for (auto& var : affineVars) {
    //    if (!util::contains(seen, var)) {
    //      reductionVars.push_back(var);
    //      seen.insert(var);
    //    }
...
WindowedIndexVar IndexVar::operator*(int stride) {
  //Setting hi as dummy.
  //These will be configured in a bound inference stage.
  return WindowedIndexVar(*this, 0, -1, stride, false);
}

WindowedIndexVar IndexVar::operator+(int lo) {
  //Setting hi as dummy.
  //These will be configured in a bound inference stage.
  return WindowedIndexVar(*this, lo, -1, 1, false);
}
...
WindowedIndexVar IndexVar::operator+(IndexVar v) {
  //Setting hi as dummy.
  //These will be configured in a bound inference stage.
  return WindowedIndexVar(*this, {v}, {1}, 0, -1, 1, true);
}
...
WindowedIndexVar IndexVar::operator+(WindowedIndexVar v) {
  IndexVar lhsVar = *this;
  IndexVar rhsBaseVar = v.getIndexVar();
  int rhsBaseStride = v.getStride();
  vector<IndexVar> rhsOffsetVars = v.getAffineLoVars();
  vector<int> rhsOffsetStrides = v.getAffineLoStrides();

  if (lhsVar == rhsBaseVar) {
    rhsBaseStride += 1;
  } else {
    size_t index = std::find(rhsOffsetVars.begin(), rhsOffsetVars.end(), lhsVar) - rhsOffsetVars.begin();
    if (index < rhsOffsetVars.size()) {
      rhsOffsetStrides[index] += 1;
    } else {
      rhsOffsetVars.push_back(lhsVar);
      rhsOffsetStrides.push_back(1);
    }
  }
  
  return WindowedIndexVar(rhsBaseVar, rhsOffsetVars, rhsOffsetStrides, 
                          v.getLowerBound(), v.getUpperBound(), rhsBaseStride,
                          rhsOffsetVars.size()>0);
}

WindowedIndexVar WindowedIndexVar::operator*(int stride) {
  this->content->stride *= stride;
  std::vector<int>& loStrides = this->content->loStrides;
  for (size_t i = 0; i < loStrides.size(); i++) {
    this->content->loStrides[i] *= stride;
  }
  return *this;
}

WindowedIndexVar WindowedIndexVar::operator+(int lo) {
  this->content->lo += lo;
  return *this;
}
...
WindowedIndexVar WindowedIndexVar::operator+(IndexVar v) {
  IndexVar lhsBaseVar = this->content->base;
  vector<IndexVar> lhsOffsetVars = this->content->loVars;
  vector<int> lhsOffsetStrides = this->content->loStrides;

  if (lhsBaseVar == v) {
    this->content->stride += 1;
  } else {
    size_t index = std::find(lhsOffsetVars.begin(), lhsOffsetVars.end(), v) - lhsOffsetVars.begin();
    if (index < lhsOffsetVars.size()) {
      this->content->loStrides[index] += 1;
    } else {
      this->content->loVars.push_back(v);
      this->content->loStrides.push_back(1);
    }
  }
  this->content->isAffine = this->content->loVars.size() > 0;
  return *this;
}
...
WindowedIndexVar WindowedIndexVar::operator+(WindowedIndexVar v) {
  IndexVar lhsBaseVar = this->content->base;
  vector<IndexVar> lhsOffsetVars = this->content->loVars;
  vector<int> lhsOffsetStrides = this->content->loStrides;
  ...
  //Lo
  this->content->lo += v.getLowerBound();
  
  // Adding rhsBaseVar
  if (lhsBaseVar == rhsBaseVar) {
    this->content->stride += rhsBaseStride;
  } else {
    size_t index = std::find(lhsOffsetVars.begin(), lhsOffsetVars.end(), rhsBaseVar) - lhsOffsetVars.begin();
    if (index < lhsOffsetVars.size()) {
      this->content->loStrides[index] += rhsBaseStride;
    } else {
      this->content->loVars.push_back(rhsBaseVar);
      this->content->loStrides.push_back(rhsBaseStride);
    }
  }

  // Adding rhsOffsetVars
  for (size_t i = 0; i < rhsOffsetVars.size(); i++) {
    IndexVar rhsOffsetVar = rhsOffsetVars[i];
    int rhsOffsetStride = rhsOffsetStrides[i];
    if (lhsBaseVar == rhsOffsetVar) {
      this->content->stride += rhsOffsetStride;
    } else {
      size_t index = std::find(lhsOffsetVars.begin(), lhsOffsetVars.end(), rhsOffsetVar) - lhsOffsetVars.begin();
      if (index < lhsOffsetVars.size()) {
        this->content->loStrides[index] += rhsOffsetStride;
      } else {
        this->content->loVars.push_back(rhsOffsetVar);
        this->content->loStrides.push_back(rhsOffsetStride);
      }
    }
  }
...
WindowedIndexVar IndexVar::operator()(int lo, int hi, int stride) {
  return WindowedIndexVar(*this, lo, hi, stride, false);
}

WindowedIndexVar IndexVar::operator()(std::vector<IndexVar> loVars, std::vector<int> loStrides, int lo, int hi, int stride) {
  return WindowedIndexVar(*this, loVars, loStrides, lo, hi, stride, true);
}
...
std::ostream& operator<<(std::ostream& os, const WindowedIndexVar& var) {
  return os << var.getIndexVar() << "(" << var.getLowerBound() << ", " << var.getUpperBound() << ", " << var.getStride() << ")" ;
}
...
WindowedIndexVar::WindowedIndexVar(IndexVar base, int lo, int hi, int stride, bool isAffine) : content( new Content){
  this->content->base = base;
  this->content->lo = lo;
  this->content->hi = hi;
  this->content->stride = stride;
  this->content->isAffine = false;
}

WindowedIndexVar::WindowedIndexVar(IndexVar base, vector<IndexVar> loVars, vector<int> loStrides, int lo, int hi, int stride, bool isAffine) : content( new Content){
  this->content->base = base;
  this->content->lo = lo;
  this->content->hi = hi;
  this->content->stride = stride;
  this->content->isAffine = true;
  this->content->loVars = loVars;
  this->content->loStrides = loStrides;
}
...
bool WindowedIndexVar::isAffineWindow() const {
  return this->content->isAffine;
}

std::vector<IndexVar> WindowedIndexVar::getAffineLoVars() const {
  return this->content->loVars;
}

std::vector<int> WindowedIndexVar::getAffineLoStrides() const {
  return this->content->loStrides;
}
...
void TensorVar::setScalar(bool isScalar) {
  content->type.setIsScalar(isScalar);
}
...
Assignment makeReductionNotation(Assignment assignment) {
  ...
  std::vector<IndexVar> free = assignment.getFreeVars();
...
IndexStmt makeConcreteNotation(IndexStmt stmt) {
  //cout << "[makeConcreteNotation - input] " << stmt << endl;
  ...
  //cout << "[makeConcreteNotation - output] " << stmt << endl << endl << endl;

  stmt = makeNotationConcordant(stmt);

  return stmt;
}

IndexStmt makeNotationConcordant(IndexStmt stmt) {
  std::vector<IndexVar> currOrder;
  match(stmt,
        std::function<void(const ForallNode*)>([&](const ForallNode* op) {
          currOrder.push_back(op->indexVar);
        })
  );

  auto dimensions = stmt.getIndexVarDomains();
  //cout << "[makeNotationConcordant] " << endl;
  //for (auto var : currOrder) {cout << var << " "; }
  //cout << endl;

  struct AffineIndexRewriter : public IndexNotationRewriter {
    using IndexNotationRewriter::visit;
    vector<IndexVar> currOrder;
    map<IndexVar, Dimension> varDomains;

    AffineIndexRewriter(vector<IndexVar> currOrder, map<IndexVar,Dimension> varDomains)
            : currOrder(currOrder), varDomains(varDomains) {}

    void visit(const AccessNode* op) {
      AccessNode* newNode = new AccessNode(op->tensorVar, op->indexVars, std::map<int,std::shared_ptr<IndexVarIterationModifier>>(), false);
      newNode->tensorVar = op->tensorVar;
      newNode->indexVars = op->indexVars;
      newNode->windowedModes = op->windowedModes;
      newNode->indexSetModes = op->indexSetModes;
      newNode->isAccessingStructure = op->isAccessingStructure;
      for (size_t i = 0; i < op->indexVars.size(); i++) {
        if (op->windowedModes.find(i) != op->windowedModes.end()) {
          AccessWindow window = op->windowedModes.at(i);
          if (window.isAffine) {
            IndexVar baseVar = op->indexVars[i];
            IndexVar newBaseVar;
            int newBaseIndex;
            bool noop = false;
            for (int j = currOrder.size()-1; j>=0; j--) {
              IndexVar varInLoop = currOrder[j];
              auto it = std::find(window.loVars.begin(), window.loVars.end(), varInLoop);
              if (it != window.loVars.end()) {
                newBaseVar = varInLoop;
                newBaseIndex = it - window.loVars.begin(); 
                break;
              } else if (varInLoop == baseVar) {
                //Nothing needs to be changed
                //cout << "[Nothing] " << op->tensorVar.getName() << " " << varInLoop << "(" << j << ") " << baseVar << endl;
                //for (auto v : window.loVars) { cout << v << " "; }
                noop = true;
                //expr = op;
                break;
              }
            ...
            if (noop) {continue;}
            //cout << "[Exchange] " << op->tensorVar.getName() << " " << i << " " << baseVar << " To " << newBaseVar << endl;
            //Swap stride
            int baseStride = window.stride;
            int newBaseStride = window.loStrides[newBaseIndex];
            newNode->windowedModes.at(i).stride = newBaseStride;
            newNode->windowedModes.at(i).loStrides[newBaseIndex] = baseStride;
            
            //Swap loVars
            newNode->windowedModes.at(i).loVars[newBaseIndex] = baseVar;
            newNode->indexVars[i] = newBaseVar;

            //Adjust hi
            int newBaseDimension = varDomains[newBaseVar].getSize();
            newNode->windowedModes.at(i).hi = newBaseDimension * newBaseStride + window.lo;
          }
        }
      }
  ...
  auto rewriter = AffineIndexRewriter(currOrder, dimensions);
  ...
  match(stmt,
        ...
        std::function<void(const AccessNode*)>([&](const AccessNode* op) {
          if (op->indexVars.size() > 0) {
            //Iterate over modes
            //cout << op->tensorVar.getName() << endl;
            for (size_t i = 0; i < op->indexVars.size(); i++) {
              //cout << op->indexVars[i];
              if (op->windowedModes.find(i) != op->windowedModes.end()) {
                //cout << "(" << op->windowedModes.at(i).lo << "," 
                //          << op->windowedModes.at(i).hi << ","
                //          << op->windowedModes.at(i).stride << ")";
              }
              //cout << ((i == op->indexVars.size()-1) ? ")" : ", ");
            }
            //cout << endl;
          }
        })
  );
  //cout << "[makeNotationConcordant End] " << endl;
  return stmt;
}
