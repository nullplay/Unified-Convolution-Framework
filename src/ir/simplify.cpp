  ...
  struct FindLoopDependentVars : public IRVisitor {
    ...
    void visit(const Assign* op) {
      if (util::contains(defLevel, op->lhs) && 
        ...
        //std::cout << "Adding " << op->lhs << " To loopDependentVars" << std::endl;
      }
    }
  };
...
//            << "with same name declared in nested scopes: " << decl->var.as<Var>()->name;
    ...
    void visit(const Var* var) {
      ...
      auto declarationMap = declarations.getMap();
      for (auto& it : declarationMap) {
        if (isa<Var>(it.first) && isa<Var>(expr) &&
            it.first.as<Var>()->name == expr.as<Var>()->name) {
          necessaryDecls.insert(it.second);
          break;
        }
      }
    }
  ...
  //std::cout << simplifiedStmt << std::endl;
  
  //std::cout << "----------- Necessary Decl ------------" << std::endl;
  //for (auto decl : copyPropagation.necessaryDecls) {
