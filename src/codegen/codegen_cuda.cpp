...
const string gpuAssertMacro =
  ...
  "  return lowerBound;\n"
  "}\n"
  "__device__ __host__ int taco_linearSearchAfter(int *array, int arrayStart, int arrayEnd, int target) {\n"
...
protected:
  ...
  virtual void visit(const For *op) {
    ...
    //op->var.accept(this);
    {
    auto lhs = op->var.as<Var>();
    varMap[op->var] = lhs->is_ptr? lhs->name : codeGen->genUniqueName(lhs->name);
    }
  ...
  virtual void visit(const Var *op) {
    if (varMap.count(op) == 0 && !inBlock) {
      /// Codegen should not assign an unique name for an affine expression.  
      bool isSameName = false;
      std::string varName;
      for (auto it : varMap) {
        if (!isa<Var>(it.first)) { continue; }
        if (it.first.as<Var>()->name == op->name) {
          isSameName = true;
          varName = it.second;
          break;
        }
...
void CodeGen_CUDA::visit(const Break* op) {
  doIndent();
  if(!isHostFunction && deviceFunctionLoopDepth == 0) {
    // can't break out of kernel
    stream << "return;" << endl;
...
void CodeGen_CUDA::visit(const Call* op) {
  ...
  if (op->args.size() > 0) {
    ...
    auto opIsBinarySearch = op->func == "taco_binarySearchAfter" || op->func == "taco_binarySearchBefore" || op->func == "taco_linearSearchAfter";
  ...
  for (size_t i=1; i < op->args.size(); ++i) {
    ...
    if (op->type != op->args[i].type() || isa<Literal>(op->args[i])) {
      if (op->func != "taco_linearSearchAfter") {
        stream << "(" << printCUDAType(op->type, false) << ") ";
      }
    }
