...
public:
  ...
  /// Returns whether or not the input mode (0-indexed) is affine windowed.
  bool isModeAffineWindowed(int mode) const;
  ...
  /// Return the {IndexVars, Strides} of the affine expression.
  std::vector<IndexVar> getAffineWindowVars(int mode) const;
  std::vector<int> getAffineWindowStrides(int mode) const;
...
public:
  WindowedIndexVar(IndexVar base, 
                   int lo = -1, int hi = -1, int stride = 1, bool isAffine=false);
  WindowedIndexVar(IndexVar base, std::vector<IndexVar> loVars, std::vector<int> loStrides, 
                   int lo = -1, int hi = -1, int stride = 1, bool isAffine=true);
  ...
  /// Interface
  WindowedIndexVar operator+(int lo) ;
  WindowedIndexVar operator-(int lo) ;
  WindowedIndexVar operator*(int stride) ;
  WindowedIndexVar operator+(IndexVar v);
  WindowedIndexVar operator-(IndexVar v);
  WindowedIndexVar operator+(WindowedIndexVar v);


  /// return whether WindowedIndexVar supports the affine expression or not.
  bool isAffineWindow() const;
  /// getAffineLo{vars,strides} returns the variable and strides of 
  /// the affine index expression.
  std::vector<IndexVar> getAffineLoVars() const;
  std::vector<int> getAffineLoStrides() const;
...
public:
  ...
  /// Indexing into an IndexVar returns a affine window into it.
  /// i({k,l}, {2,3}, 1, 4) means 4*i + (2*k + 3*l + 1)
  WindowedIndexVar operator()(std::vector<IndexVar> loVars, std::vector<int> loStrides, 
                              int lo, int hi, int stride = 1);
...
struct WindowedIndexVar::Content {
  ...
  bool isAffine; 
  std::vector<IndexVar> loVars;
  std::vector<int> loStrides;
};
...
public:
  ...
  /// set virtualScalar in tensor Type.
  void setScalar(bool isScalar);
...
/// match affine index variables along with loop order.
