...
#define   REGEN
#ifdef REGEN
  source_file.open(prefix_path+"tmp3dgpu"+suffix+".cu");
...
#ifdef LIDAR2
  string cflag = " -maxrregcount=64  -w -O3 -Xcompiler \"-fPIC -shared -ffast-math -O3\" --generate-code arch=compute_70,code=sm_70 ";
#else
  string cflag = " -maxrregcount=128  -w -O3 -Xcompiler \"-fPIC -shared -ffast-math -O3\" --generate-code arch=compute_70,code=sm_70 ";
#endif

  string sourcename = prefix_path+"tmp3dgpu"+suffix+".cu ";
  string shimname = prefix_path+"tmp3dgpu_shim"+suffix+".cpp ";
  string objectname = prefix_path+"tmp3dgpu"+suffix+".so";
  ...
  lib_handle = dlopen("./tmp3dgpu.so", RTLD_NOW|RTLD_LOCAL);
  if (!lib_handle) {
    fprintf(stderr, "%s\n", dlerror());
    cout << "ERR" << endl;
  }
...
int main(int argc, char* argv[]) {
  ...
  cout << "X" << endl;
  bool isCuda = true;
  set_CUDA_codegen_enabled(isCuda);
  set_CUDA_unified_memory_enabled(isCuda);

#if defined LIDAR1 || LIDAR2
  ...
  cout << P << " " << Q << " " << U <<  " " << crds.size() << " " << I.getStorage().getSizeInBytes() << endl;
...
Out(p,q,u,m) = Mask(p,q,u) * I(p+r-R/2, q+s-S/2, u+t-T/2, c) * F(r,s,t,c,m);
  ...
  for (int i = 0; i<rep; i++) func(O.getTacoTensorT(), I.getTacoTensorT(), I.getTacoTensorT(), F.getTacoTensorT());
  auto start = Clock::now();
  for (int i = 0; i<rep; i++) func(O.getTacoTensorT(), I.getTacoTensorT(), I.getTacoTensorT(), F.getTacoTensorT());
  auto end = Clock::now();
  cout << compute_clock(end, start)/rep << endl;
